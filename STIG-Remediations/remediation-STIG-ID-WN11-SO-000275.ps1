<#
.SYNOPSIS
    Enables UAC virtualization of file and registry write failures to
    per-user locations, so legacy applications that write to protected
    system areas are redirected instead of silently failing or requiring
    elevation.
...
.NOTES
    ...
    Vuln-ID         : V-253475
    CCI             : CCI-001084
    Documentation   : https://stigaview.com/products/win11/v2r8/WN11-SO-000275/
...
.USAGE
    ...
    Example syntax:
    PS C:\> .\remediation-STIG-ID-WN11-SO-000275.ps1
    ...
    Run "gpupdate /force"; a logoff/logon may be needed for the change to be reflected everywhere.

    Rollback (returns the VM to a non-compliant / default state for lab evidence):
    Remove-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" -Name "EnableVirtualization" -ErrorAction SilentlyContinue
#>
