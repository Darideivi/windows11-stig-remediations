<#
.SYNOPSIS
    Disables Basic authentication on the WinRM client, preventing WinRM from
    sending credentials in an unencrypted (Base64) form over the network.
...
.USAGE
    ...
    Example syntax:
    PS C:\> .\remediation-STIG-ID-WN11-CC-000330.ps1
    ...
    Run "gpupdate /force"; restarting the WinRM service (Restart-Service WinRM) is
    recommended so the client picks up the change immediately.

    Rollback (returns the VM to a non-compliant / default state for lab evidence):
    Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WinRM\Client" -Name "AllowBasic" -ErrorAction SilentlyContinue
#>
